...
export const submitHouseForApproval = async (id) => {
  try {
    const response = await api.post(`${API_ENDPOINTS.HOUSES}${id}/submit_for_approval/`)
    return response.data
  } catch (error) {
    throw error.response?.data || { message: "Failed to submit house for approval" }
  }
}
...
export const addReview = async (houseId, reviewData) => {
  try {
    const response = await api.post(`${API_ENDPOINTS.HOUSES}${houseId}/add_review/`, reviewData)
    return response.data
  } catch (error) {
